...
import { requestGmailAccessToken, sendGmailMessageWithPdfAttachment } from "../services/gmailDraftService";
...
export default function MaintenancePage() {
  ...
  const isManagementPage = location.pathname === "/maintenance/manage";
  ...
  useEffect(() => {
    ...
    const unsubscribes = clients.map((client) =>
      subscribeMaintenanceReportHistory(
        client.id,
        (items) => {
          setReportHistoryByClient((prev) => ({ ...prev, [client.id]: items }));
          setLiveStatsError("");
        },
        (err) => {
          console.error(err);
  ...
  async function handleGenerateReport(type: ReportType) {
    ...
    try {
      gmailAccessToken = await requestGmailAccessToken(gmailSenderEmail.trim());
    } catch (err) {
      console.error(err);
      setReportGenerating(false);
      setReportError("Autorizarea Gmail a fost blocata sau refuzata. Permite popup-uri si selecteaza contul Gmail.");
      setReportMessage("");
      return;
    }
    ...
    try {
      ...
      const sentMessage = await sendGmailMessageWithPdfAttachment({
        ...
        senderEmail: gmailSenderEmail.trim(),
        ...
        <div className="maintenance-report-steps">
          ...
          <div className="panel maintenance-step-card maintenance-step-card--send" data-assistant-step="report-send">
            ...
            <div className="tool-form-block">
              ...
              <input className="tool-input" data-assistant-field="maintenance-report-sender" value={gmailSenderEmail} onChange={(e) => setGmailSenderEmail(e.target.value)} placeholder="exemplu@gmail.com" />
              <div className="simple-list-subtitle">Gmail se va deschide pe acest cont daca este deja logat in browser.</div>
            </div>
  ...
  void renderLegacyMaintenancePage;
  ...
  return (
    <section className="page-section maintenance-page">
      ...
      {renderActiveMaintenanceTab()}
    </section>
  );

  function renderLegacyMaintenancePage() {
    return (
    <section className="page-section maintenance-page">
      <div className="panel maintenance-toolbar" style={{ order: -10 }}>
        <div>
          <h1 className="panel-title" style={{ marginBottom: 4 }}>
            {isManagementPage ? "Administrare mentenanta" : "Mentenanta"}
          </h1>
          <p className="tools-subtitle">
            {isManagementPage
               ? "Formular client, branding firma si lista clientilor de mentenanta."
              : "Generare raport, verificare revizii si status live."}
          </p>
        </div>
        {isManagementPage ? (
          <Link className="secondary-btn" to="/maintenance">
            Inapoi la generare raport
          </Link>
        ) : (
          <div className="maintenance-actions">
            <Link className="secondary-btn" data-assistant-action="maintenance-parts" to="/maintenance/orders">
              Comenzi piese
            </Link>
            <Link className="primary-btn" to="/maintenance/manage">
              Formulare si clienti mentenanta
            </Link>
          </div>
        )}
      </div>
      {isManagementPage && (
      <div className="kpi-grid" style={{ order: 5 }}>
        <div className="kpi-card">
      ...
      {isManagementPage && (
      <details className="panel" data-assistant-action="maintenance-branding" style={{ order: 3 }}>
        <summary className="panel-title">Branding firma</summary>
      ...
      {isManagementPage && (
      <details id="maintenance-client-form" className="panel" data-assistant-action="maintenance-add-client" style={{ order: 2 }} open={assistantClientFormOpen}>
        <summary className="panel-title">Formular client</summary>
        ...
        <div className="tool-form-grid" style={{ marginTop: 12 }}>
          <div className="tool-form-block">
            ...
            <input
              className="tool-input"
              data-assistant-field="maintenance-client-name"
          ...
          <div className="tool-form-block">
            ...
            <input
              className="tool-input"
              data-assistant-field="maintenance-client-email"
          ...
          <div className="tool-form-block">
            ...
            <input
              className="tool-input"
              data-assistant-field="maintenance-client-contact-person"
          ...
          <div className="tool-form-block">
            ...
            <input
              className="tool-input"
              data-assistant-field="maintenance-client-contact-phone"
          ...
          <div className="tool-form-block" style={{ gridColumn: "1 / -1" }}>
              ...
              {clientForm.addresses.map((address, addressIndex) => (
                <div className="simple-list-item" key={address.id} style={{ alignItems: "stretch" }}>
                  <div className="simple-list-text">
                    ...
                    <input
                      className="tool-input"
                      data-assistant-field="maintenance-client-address"
                      value={address.address}
                      onChange={(e) => updateClientFormAddress(address.id, e.target.value)}
                      ...
                      {address.lifts.map((lift, liftIndex) => (
                        <div className="simple-list-item" key={lift.id}>
                          <div className="simple-list-text">
                            ...
                            <div className="tool-form-grid">
                              <div className="tool-form-block">
                                ...
                                <input
                                  className="tool-input"
                                  data-assistant-field="maintenance-client-lift-number"
                                  value={lift.liftNumber}
                                  onChange={(e) =>
                                    updateClientFormLift(address.id, lift.id, "liftNumber", e.target.value)
                                  }
                                  placeholder="210869"
                                />
                              </div>
                              <div className="tool-form-block">
                                ...
                                <input
                                  className="tool-input"
                                  data-assistant-field="maintenance-client-expiry-date"
                                  type="date"
                                  value={lift.expiryDate}
                                  onChange={(e) =>
                                    updateClientFormLift(address.id, lift.id, "expiryDate", e.target.value)
                                  }
                                />
                              </div>
                              <div className="tool-form-block">
                                ...
                                <select
                                  className="tool-input"
                                  data-assistant-field="maintenance-client-revision-type"
                                  value={lift.revisionType}
                                  onChange={(e) =>
                                    updateClientFormLift(address.id, lift.id, "revisionType", e.target.value)
                                  }>
                                  <option value="R2">R2</option>
                                  <option value="R1">R1</option>
                                </select>
                          ...
                          <button
                            ...
                            type="button"
                            onClick={() => removeClientFormLift(address.id, lift.id)}
                            disabled={address.lifts.length <= 1}>
                            Sterge lift
                          </button>
                        </div>
                      ))}
                    ...
                    <div className="maintenance-actions" style={{ marginTop: 10 }}>
                      <button className="secondary-btn" type="button" onClick={() => addClientFormLift(address.id)}>
                        + Lift
                      </button>
                      <button
                        className="danger-btn"
                        type="button"
                        onClick={() => removeClientFormAddress(address.id)}
                        disabled={clientForm.addresses.length <= 1}>
                        Sterge adresa
                      </button>
                    </div>
                  </div>
            ...
            <div className="tool-form-actions" style={{ marginTop: 10 }}>
              <button className="secondary-btn" type="button" onClick={addClientFormAddress}>
                + Adresa
              </button>
            </div>
          </div>
          <div className="tool-form-block">
            ...
            <input
              className="tool-input"
              data-assistant-field="maintenance-client-company"
      ...
      <div
        ...
        style={{
          ...
          <div>
            <div className="panel-title">Status revizii si expirari</div>
            <div className="tools-subtitle">
              Mai sunt {liveMonthlyMissingReviews.length}/{liveTotalLifts} revizii de facut ({liveMonthlyMissingReviews.length} lifturi fara revizie).
            </div>
            {liveStatsError && (
              <div className="tool-message" style={{ marginTop: 8 }}>
                {liveStatsError}
              </div>
          )}
        </div>
        <div className="kpi-grid" style={{ minWidth: 260 }}>
          <div className="kpi-card">
            <div className="kpi-label">Revizii luna curenta</div>
            <div className="kpi-value">
              {liveMonthlyMissingReviews.length}/{liveTotalLifts}
            </div>
          </div>
          <div className="kpi-card">
            <div className="kpi-label">Expirate / expira curand</div>
        ...
        <div className="tools-subtitle" style={{ width: "100%" }}>
          Sunt {liveExpiredAndNextMonthExpiringLifts.length} lifturi expirate deja sau care expira luna urmatoare.
        </div>
        {liveExpiredAndNextMonthExpiringLifts.length > 0 && (
          <details style={{ width: "100%" }}>
            <summary className="panel-subtitle" style={{ cursor: "pointer" }}>
              Lista lifturi expirate / care expira
            </summary>
            <div className="simple-list" style={{ marginTop: 12, maxHeight: 260, overflowY: "auto" }}>
              {liveExpiredAndNextMonthExpiringLifts.map((item) => (
                <div
                  className="simple-list-item"
                  key={`expiring_live_${item.clientId}_${item.address}_${item.lift}`}>
                  <div className="simple-list-text">
                    <div className="simple-list-label">Lift: {item.lift} · Exp. Date: {item.expiryDate}</div>
                    <div className="simple-list-subtitle">Client: {item.clientName}</div>
                    <div className="simple-list-subtitle">Adresa: {item.address}</div>
                    <div className="simple-list-subtitle">Firma mentenanta: {item.maintenanceCompany}</div>
                  </div>
                  <button
                    className="secondary-btn"
                    type="button"
                    onClick={() => navigate(`/maintenance/${item.clientId}`)}>
                    Detalii
                  </button>
                </div>
              ))}
      ...
      <div id="maintenance-report-generator" className="panel" data-assistant-section="maintenance-report-generator" style={{ order: 1 }}>
        ...
        {lastGeneratedReport && (
          <div className="simple-list" style={{ marginTop: 12 }}>
            <div className="simple-list-item">
              <div className="simple-list-text">
                <div className="simple-list-label">Email trimis catre {lastGeneratedReport.clientName || "client"}</div>
                <div className="simple-list-subtitle">Destinatar: {lastGeneratedReport.clientEmail || "-"}</div>
                <div className="simple-list-subtitle">PDF-ul a fost trimis ca atasament.</div>
              </div>
              <div className="maintenance-actions">
                {lastGeneratedReport.pdfUrl ? (
                  <button
                ...
                <button
                  ...
                  type="button"
                  onClick={() => window.open(lastGeneratedReport.gmailUrl, "_blank")}>
                  Deschide Gmail Sent
                </button>
              </div>
            </div>
        ...
        <div className="tool-form-grid" style={{ marginTop: 12 }}>
          ...
          <div className="tool-form-block" style={{ gridColumn: "1 / -1" }}>
            ...
            <div className="simple-list-subtitle">
              Gmail se va deschide pe acest cont daca este deja logat in browser.
            </div>
          </div>
          ...
          <div className="tool-form-block">
            ...
            <select
              ...
              <option value="">Selecteaza adresa</option>
              {reportAddressOptions.map((address) => (
                <option key={`report_address_${address}`} value={address}>
                  {address}
                </option>
              ))}
            </select>
          ...
          <div className="tool-form-block">
            ...
            <select
              className="tool-input"
              value={reportLift}
              onChange={(e) => setReportLift(e.target.value)}
              disabled={!selectedClient}>
              <option value="">Selecteaza liftul</option>
              {reportLiftOptions.map((lift) => (
                <option key={`report_lift_${lift}`} value={lift}>
                  {lift}
                </option>
              ))}
            </select>
          ...
          <div className="tool-form-block" style={{ gridColumn: "1 / -1" }}>
            ...
            <input
              ...
              accept="image/*"
              multiple
              onChange={(e) => setReportImageFiles(Array.from(e.target.files || []))}
            />
            <div className="simple-list-subtitle">
              {reportImageFiles.length ? `${reportImageFiles.length} poze selectate` : "Nu ai selectat poze."}
            </div>
          </div>

          <div className="tool-form-block" style={{ gridColumn: "1 / -1", position: "relative" }}>
            <label className="tool-form-label">Tehnician (minim 2 litere)</label>
            <input
              className="tool-input"
            ...
            {technicianSuggestions.length > 0 && technicianSearch.trim().length >= 2 && !selectedTechnician && (
              <div className="simple-list" style={{ marginTop: 8, maxHeight: 220, overflowY: "auto" }}>
                {technicianSuggestions.map((user) => (
                  <button
                    key={`technician_suggestion_${user.id}`}
                    type="button"
                    className="simple-list-item"
                    onClick={() => selectTechnician(user)}
                    style={{ width: "100%", textAlign: "left", cursor: "pointer" }}>
                    <div className="simple-list-text">
                      <div className="simple-list-label">{user.fullName}</div>
                      <div className="simple-list-subtitle">{user.email || "-"}</div>
                    </div>
                  </button>
                ))}
              </div>
      ...
      <div className="panel" style={{ order: 1 }}>
        <h2 className="panel-title">Verificare revizie lunara lift</h2>
        <p className="tools-subtitle">
          Verifica fiecare numar de lift din toti clientii si arata lifturile fara raport de revizie emis in luna curenta.
        </p>
        ...
        <div className="tool-form-actions" style={{ marginTop: 14 }}>
          <button
            className="primary-btn"
            type="button"
            onClick={() => void handleCheckMonthlyReviews()}
            disabled={monthlyReviewChecking || loading}>
            {monthlyReviewChecking ? "Se verifica..." : "Verifica revizii luna curenta"}
          </button>
        </div>

        {monthlyReviewMissing && (
          <div style={{ marginTop: 16 }}>
            {monthlyReviewMissing.length === 0 ? (
              <h3 className="panel-subtitle">Toate lifturile au revizie luna aceasta.</h3>
            ) : (
              <details>
                <summary className="panel-subtitle" style={{ cursor: "pointer" }}>
                  Mai sunt {monthlyReviewMissing.length} lifturi fara revizie.
                </summary>
                <div className="simple-list" style={{ marginTop: 12, maxHeight: 360, overflowY: "auto" }}>
                  {monthlyReviewMissing.map((item) => (
                    <div
                      className="simple-list-item"
                      key={`missing_review_${item.clientId}_${item.address}_${item.lift}`}>
                      <div className="simple-list-text">
                        <div className="simple-list-label">Lift: {item.lift}</div>
                        <div className="simple-list-subtitle">Client: {item.clientName}</div>
                        <div className="simple-list-subtitle">Adresa: {item.address}</div>
      ...
      {isManagementPage && (
      <details className="panel" data-assistant-action="maintenance-history" style={{ order: 4 }}>
        <summary className="panel-title">Clienti mentenanta</summary>
        ...
        {loading ? (
          ...
          <div className="simple-list" style={{ maxHeight: 520, overflowY: "auto" }}>
            {filteredClients.map((client) => {
              ...
              return (
                <div
                        ...
                        {group.lifts.length ? (
                          group.lifts.map((lift) => (
                            <div key={`${group.key}_lift_${lift}`} className="simple-list-subtitle">
                              • Lift: {lift} · Exp. Date: {getLiftExpiryDate(client, lift)}
                            </div>
                          ))
                        ) : (
                  ...
                  <div className="maintenance-actions">
                    <button className="secondary-btn" type="button" onClick={() => navigate(`/maintenance/${client.id}`)}>
                      Detalii
                    </button>
                    <button className="danger-btn" type="button" onClick={() => void handleDeleteClient(client)}>
                      Sterge client
