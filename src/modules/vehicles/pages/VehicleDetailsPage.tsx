...
import { AlertTriangle, Pencil, ArrowLeft, Image as ImageIcon, History } from "lucide-react";
...
/** Skeleton shown while vehicle loads */
function VehicleDetailSkeleton() {
  return (
    <section className="page-section">
      <div className="panel">
        <div className="tool-details-header" style={{ gap: 16, padding: "4px 0" }}>
          <div className="skeleton" style={{ width: 120, height: 120, borderRadius: "var(--radius-md)", flexShrink: 0 }} />
      ...
      <div className="tool-details-grid" style={{ marginTop: 16 }}>
        {[1, 2, 3].map((i) => (
          <div key={i} className="panel" style={{ minHeight: 180 }}>
            ...
            {[1, 2, 3, 4].map((j) => (
              <div key={j} className="skeleton" style={{ height: 12, marginBottom: 10, width: `${70 - j * 8}%` }} />
            ))}
          </div>
        ))}
      </div>
...
export default function VehicleDetailsPage() {
  ...
  useEffect(() => {
    ...
    return () => {
      try { unsubscribe?.(); } catch (err) {
        console.error("[VehicleDetailsPage][unsubscribe]", err);
      }
    };
  }, [vehicleId]);
  ...
  const loadMeta = useCallback(async () => {
    ...
    try {
      ...
      if (!mountedRef.current) return;
      setEvents([]); setUsers([]); setCommands([]);
    } finally {
      if (mountedRef.current) setMetaLoading(false);
    }
  }, [vehicleId]); // metaLoading intentionally excluded to avoid stale closure

  useEffect(() => {
    if (vehicleId) void loadMeta();
  }, [vehicleId]); // loadMeta is stable via useCallback
  ...
  async function handleRequestCommand(type: "pulse_dout1" | "block_start") {
    ...
    const latest = await getVehicleCommands(vehicle.id).catch(() => []);
    if (mountedRef.current) setCommands(latest);
  }
  ...
  // Maintenance alerts for expiring docs
  ...
  if (!vehicle) {
    return (
      <div className="placeholder-page">
        ...
        <p>Verifică link-ul sau lista mașinilor.</p>
        <Link to="/vehicles" className="secondary-btn" style={{ marginTop: 16, display: "inline-flex" }}>
          <ArrowLeft size={15} style={{ marginRight: 6 }} /> Înapoi la mașini
        </Link>
      </div>
    );
  }
    ...
    <section className="page-section">
      ...
      <div className="panel">
        ...
        <div className="tools-header">
          <div className="tool-details-header">
            ...
            <div>
              <h2 className="panel-title" style={{ marginBottom: 6 }}>{vehicle.plateNumber}</h2>

              <div className="tool-detail-line">
                <strong>Marcă / model:</strong> {vehicle.brand} {vehicle.model}
                {vehicle.year ? <span style={{ color: "var(--text-muted)", marginLeft: 6 }}>· {vehicle.year}</span> : null}
              </div>
              ...
              <div className="tool-detail-line">
                <strong>Șofer curent:</strong>{" "}
                {vehicle.currentDriverUserName || <span style={{ color: "var(--text-muted)" }}>Neasignat</span>}
              </div>

              <div className="tool-detail-line">
                <strong>Km curenți:</strong>{" "}
      ...
      {needsRepair && user && (
        <div className="panel">
          <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
            ...
            <div>
              <h3 className="panel-title" style={{ marginBottom: 4 }}>Date incomplete</h3>
              <p className="tools-subtitle" style={{ marginBottom: 12 }}>
                Această mașină nu are responsabil setat. O poți prelua pe profilul tău.
              </p>
              {claimMsg && (
                <p style={{ marginBottom: 10, fontSize: 13, color: claimMsg.includes("succes") ? "var(--success)" : "var(--danger)" }}>
                  {claimMsg}
                </p>
              )}
              <button
      ...
      <div className="tool-details-grid">
        ...
        <div className="panel tool-inner-panel">
          ...
          <div className="tool-detail-line">
            <strong>An fabricație:</strong> {vehicle.year || "—"}
          </div>
          <div className="tool-detail-line">
            <strong>VIN:</strong>{" "}
            <span style={{ fontFamily: "monospace", fontSize: 12, letterSpacing: "0.5px" }}>
              {vehicle.vin || "—"}
            </span>
          </div>
          <div className="tool-detail-line">
          ...
          <div className="tool-detail-line">
            <strong>Km curenți:</strong> {(vehicle.currentKm || 0).toLocaleString("ro-RO")} km
          </div>
          <div className="tool-detail-line">
            <strong>Km la înregistrare:</strong> {(vehicle.initialRecordedKm || 0).toLocaleString("ro-RO")} km
          </div>
        </div>

        {/* Mentenanță */}
        <div className="panel tool-inner-panel">
          <h3 className="panel-title">Mentenanță</h3>

          <div className="tool-detail-line">
            <strong>Următor service la:</strong>{" "}
            {vehicle.nextServiceKm ? `${vehicle.nextServiceKm.toLocaleString("ro-RO")} km` : "—"}
          </div>
          <div className="tool-detail-line">
            <strong>ITP până la:</strong>{" "}
            <span style={{ color: getDateColor(vehicle.nextItpDate) }}>
              {vehicle.nextItpDate || "—"}
            </span>
          </div>
          <div className="tool-detail-line">
            <strong>RCA până la:</strong>{" "}
            <span style={{ color: getDateColor(vehicle.nextRcaDate) }}>
              {vehicle.nextRcaDate || "—"}
            </span>
          </div>
          <div className="tool-detail-line">
            <strong>CASCO până la:</strong>{" "}
            <span style={{ color: getDateColor(vehicle.nextCascoDate) }}>
              {vehicle.nextCascoDate || "—"}
            </span>
          </div>
          {vehicle.maintenanceNotes && (
            <div className="tool-detail-line" style={{ flexDirection: "column", alignItems: "flex-start", gap: 4 }}>
              <strong>Note mentenanță:</strong>
              <span style={{ color: "var(--text-soft)", fontSize: 13, lineHeight: 1.5 }}>
                {vehicle.maintenanceNotes}
              </span>
            </div>
          )}
      ...
      {/* ── GPS MAP ── */}
      <SectionErrorBoundary sectionName="hartă GPS">
      ...
      <div className="panel">
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          ...
          <span style={{ fontSize: 12, color: "var(--text-muted)" }}>
            {vehicle.images.length} {vehicle.images.length === 1 ? "poză" : "poze"}
          </span>
        </div>

        {vehicle.images.length === 0 ? (
          <div className="empty-state">
            <div className="empty-state-icon"><ImageIcon size={20} strokeWidth={1.6} /></div>
            <div className="empty-state-title">Nicio poză încărcată</div>
            <div style={{ fontSize: 13, color: "var(--text-muted)" }}>
              {isOwner ? "Editează vehiculul pentru a adăuga poze." : "Nicio poză disponibilă."}
            </div>
          </div>
        ) : (
            ...
            {vehicle.images.map((image) => (
              <div key={image.id} className="tool-gallery-item">
                <img
                  ...
                  alt={image.fileName || "Poză vehicul"}
                  className="tool-gallery-image"
                  loading="lazy"
                  decoding="async"
                  onError={(e) => {
                    // fallback to full URL if thumb fails
                ...
                {isOwner && (
                  <div className="tool-gallery-actions">
                    ...
                    >
                      {deletingImageId === image.id ? "..." : "Șterge"}
                    </button>
                  </div>
                )}
              </div>
      ...
      <div className="panel">
        ...
        {events.length === 0 ? (
          <div className="empty-state">
            <div className="empty-state-icon"><History size={20} strokeWidth={1.6} /></div>
            <div className="empty-state-title">Niciun eveniment înregistrat</div>
          </div>
        ) : (
...
function getDateColor(dateStr?: string): string {
  ...
  if (isNaN(d.getTime())) return "inherit";
