...
describe("dashboardService cost bounds", () => {
  ...
  it("loads only the selected day and limits recent notifications", async () => {
    ...
    const notificationQuery = firestoreMocks.query.mock.calls.find(([base]) => base === "notifications");
    ...
    expect(firestoreMocks.query.mock.calls.filter(([base]) => base === "timesheets")).toHaveLength(2);
    expect(firestoreMocks.query.mock.calls.filter(([base]) => base === "notifications")).toHaveLength(2);
  });
});
