...
function normalizeColloquialRomanian(value: string) {
  ...
  const replacements: Array<[RegExp, string]> = [
    ...
    [/^\s*(?:am\s+nevoie|as\s+avea\s+nevoie|trebuie)(?:\s+si\s+eu)?\s+(?:sa\s+)?/giu, ""],
    [
      /^\s*(?:ia|hai|vezi)(?:\s+te\s+rog)?\s+(?=(?:fa|pune|schimba|modifica|deschide|arata)\b)/giu,
      "",
    ],
    [/\b(?:imi|mi)\s+(?:deschizi|arati)\b/giu, "deschide"],
    [/\bdeschizi\b/giu, "deschide"],
    [/\barati\b/giu, "arata"],
    [/\bmodifici\b/giu, "modifica"],
    [/\bschimbi\b/giu, "schimba"],
    [/\bpui\b/giu, "pune"],
    [/\bsetezi\b/giu, "seteaza"],
    [/\bactualizezi\b/giu, "actualizeaza"],
    [
    ...
    [/\b(modifica|schimba|seteaza|actualizeaza)\s*-?\s*(?:l|o|i)\b/giu, "$1"],
    ...
    [/\bmi\s*-\s*am\s+terminat\s+treaba\b/giu, "opreste pontajul"],
    [
      /\b(?:gata\s+(?:cu|pe)\s+pontaju(?:l)?|gata\s+pe\s+azi|am\s+(?:terminat|plecat)(?:\s+pe\s+azi)?|inchide\s+(?:ziua|programul))\b/giu,
      "opreste pontajul",
    ],
    [
      /\b(?:am\s+gatat|mi\s*-?\s*am\s+terminat\s+treaba|s\s*-?\s*a\s+terminat\s+programul|nu\s+mai\s+lucrez)\b/giu,
      "opreste pontajul",
    ],
    ...
    [/\bunde\s+sunt\s+(?:toate\s+)?masinile\b/giu, "deschide toate gps-urile"],
    [/\b(masina|masinii|scula|proiectul|pontajul)\s+lu\b/giu, "$1 lui"],
    [/\b(?:dubita|autoutilitara|utilitara)\b/giu, "duba"],
    [/\b(?:santieru|santierul|lucrarea|lucrare)\b/giu, "proiectul"],
    [/\b(?:colegu|salariatu|muncitoru)\b/giu, "utilizatorul"],
    ...
    [/\b(?:dupa\s+asta|pe\s+urma|dupa\s+care)\b/giu, "si apoi"],
  ];
