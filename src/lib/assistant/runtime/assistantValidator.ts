...
export function validateAssistantPlan(plan: AssistantRuntimePlan, roleContext: Parameters<typeof checkAssistantPermission>[0]): AssistantValidationResult {
  ...
  for (const change of plan.changes) {
    if (plan.entityType === "vehicle" && change.fieldKey === "currentKm" && Number(change.newValue) < 0) {
      return { ok: false, message: "Km curenti trebuie sa fie un numar pozitiv." };
    }
    if (plan.entityType === "vehicle" && change.fieldKey === "status" && !VALID_VEHICLE_STATUSES.has(String(change.newValue))) {
      return { ok: false, message: "Statusul masinii nu este valid." };
    }
    if (plan.entityType === "tool" && change.fieldKey === "status" && !VALID_TOOL_STATUSES.has(String(change.newValue))) {
      return { ok: false, message: "Statusul sculei nu este valid." };
    }
    if (plan.entityType === "project" && change.fieldKey === "status" && !VALID_PROJECT_STATUSES.has(String(change.newValue))) {
      return { ok: false, message: "Statusul proiectului nu este valid." };
    }
    if (isDateField(change.fieldKey) && !validIsoDate(change.newValue)) {
      return { ok: false, message: `Data pentru ${change.label} trebuie sa fie in format YYYY-MM-DD.` };
    }
  }
  ...
  const needsSpecialConfirmation = plan.changes.some((change) => change.requiresSpecialConfirmation);
  return {
    ok: true,
