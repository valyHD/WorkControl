...
const { buildVehicleAlertScheduleSources } = require("../functions/expiryAutomation.js");
...
async function main() {
  ...
  do {
    let vehiclesQuery = db.collection("vehicles").orderBy(FieldPath.documentId()).limit(pageSize);
    if (cursor) vehiclesQuery = vehiclesQuery.startAfter(cursor);
    const vehicles = await vehiclesQuery.get();
    if (vehicles.empty) break;
    vehiclesScanned += vehicles.size;
    cursor = vehicles.docs.at(-1);

    const desiredSchedules = vehicles.docs.flatMap((vehicle) =>
      buildVehicleAlertScheduleSources(vehicle.id, vehicle.data() || {}, now)
    );
    ...
    if (vehicles.size < pageSize) break;
  } while (cursor);
