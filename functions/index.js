...
function buildAssistantPrompt(today, context) {
  ...
  return [
    ...
    'Tool id-uri permise: entities.read, navigation.open, vehicles.open, vehicles.update, vehicles.draft, tools.update, tools.draft, timesheets.projects.update, timesheets.projects.create, timesheets.projects.draft, users.update, users.draft, timesheets.start, timesheets.stop, maintenance.draft, maintenance.report.prepare, maintenance.report.send, leave.draft, expenses.draft, notifications.rules.update, settings.update.',
    ...
    'Pentru intrebari despre date existente foloseste commandType question, intent read_entity si tool entities.read. Pune in fields doar cheile cerute cu valoarea true; nu emite update si nu cere confirmare.',
    'Pentru "masina mea" foloseste entityType vehicle si entityQuery "__current_vehicle__". Pentru "profilul meu" foloseste entityType user si entityQuery "__current_user__"; resolverul foloseste utilizatorul autentificat.',
    'Exemple de citire: "cati km am" => vehicle/currentKm; "cine conduce Toyota" => vehicle/driver; "unde e scula Bosch" => tool/locationLabel; "ce functie are Mihai" => user/roleTitle; "ce status are proiectul Service 2" => project/status.',
